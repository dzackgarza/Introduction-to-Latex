...
No one likes to repeat themselves! Typing code into a document can be error-prone, and must be manually changed if the code is updated. If you already have a program's source in a file, its entire text content of the file can be imported and styled automatically.

\begin{example}
    Here we import an Octave script directly from a file:
    \inputminted{Octave}{examplecode.m}
\end{example}
...
\begin{fact}
    Linear optimization is essentially just a matter of systematically looking for minima or maxima among the vertices of a convex polygon in $\RR^n$. In this 2-dimensional case, the function $f$ (a single, blue level curve is included in the plot) is maximized at the point $A$:
